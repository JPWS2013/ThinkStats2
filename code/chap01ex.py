...
def MakePregNumReg(df):
    """Make a map from caseid to number of pregnancies reported

    df: DataFrame

    returns: dict that maps from caseid to number of pregnancies reported in df
    """
    d = defaultdict(list)
    for index, pregnum in df.pregnum.iteritems():
        d[df.caseid[index]]=pregnum
    return d
...
if __name__ == '__main__':
    ...
    df2=ReadFemResp()
    print (df2.pregnum.value_counts().sort_index())
    
    errorlist=[]
    
    PregNumReg=MakePregNumReg(df2)
    #print(PregNumReg)
    preg_map=nsfg.MakePregMap(df1)

    for caseid, pregnum in PregNumReg.iteritems():
